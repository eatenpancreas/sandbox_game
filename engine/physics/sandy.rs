use bevy::math::UVec2;
use bevy::prelude::{Component, EventWriter, Query, Res};
use rand::Rng;
    ...
    pub fn physics(
        ...
        for (pos, phys) in q_phys.iter() {
            ...
            if (phys.do_move_on_ground && rng.gen_range(0..4) > 1) {
                if !do_sand_drop_to_sides(&mut set_pixel, &q_pass, &grid, phys, pos, rn, pos.0.y) && new_y > pos.0.y {
...
fn do_sand_drop_to_sides(
    ...
    if let Some(x) = if (0.0..=phys.disperse_chance).contains(&rn) {
        ...
        return sand_drop(set_pixel, &q_pass, &grid, pos.0, phys.weight, rn, x, y);
    }
    false
}
